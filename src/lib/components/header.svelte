...
<header class="flex items-center justify-between p-4 bg-zinc-200 dark:bg-zinc-800">
	...
	<nav class="flex space-x-4">
		...
		<div class="relative inline-block">
            ...
            id="themeToggler" on:click={toggleDarkMode}>🌓
			</button>
		</div>
	</nav>
	...
	<div class="flex items-center relative">
		<button  class="w-10 h-10 rounded-full bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700"
			id="profileDropdown" on:click={toggleDropdown}>👤
		</button>
		{#if $showDropdown}
			<div class="origin-top-right absolute right-0 top-0 mt-12 w-40 rounded-md shadow-lg bg-white dark:bg-zinc-800 ring-1 ring-black ring-opacity-5 focus:outline-none"
